...
<head>
    <meta charset = 'utf-8'>
    <title>다크웹 크롤링 테스트 (이미지 데이터)</title>
</head>
<style>
    table{
...
<h2 align=center><a href = "./crawlingtest_img.php">다크웹 크롤링 테스트 (이미지 데이터)</a></h2>
<table align = center>
    <thead align = "center">
    <tr>
        <td width = "50" align="center">No</td>
        <td width = "200" align = "center">TITLE</td>
        <td width = "200" align = "center">IMAGE DATA</td>
    ...
    $sql_page = "SELECT * FROM img limit $start, $per";
    ...
    while($row = mysqli_fetch_array($res_page)){ //DB에 저장된 데이터 수 (열 기준)
        ...
        <td width=420 align="center" style="word-break:break-all"><?php echo $row['img']?></td>
    ...
    if($page > 1){
        $pre = $page - 1;
        echo "<a href=\"crawlingtest_img.php?page=$pre\">이전 </a>";
    }
    ...
    while($page_num <= $total_page){
        if($page==$page_num){
            echo "<a style=\"color:purple;\" href=\"crawlingtest_img.php?page=$page_num\">$page_num </a>";
        } else {
            echo "<a href=\"crawlingtest_img.php?page=$page_num\">$page_num </a>"; }
        $page_num++;
    }

    if($page < $total_page){
        $next = $page + 1;
        echo "<a href=\"crawlingtest_img.php?page=$next\">다음 </a>";
    }
    if($page < $total_page){
